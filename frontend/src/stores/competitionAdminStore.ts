...
export function competitionAdminStore(initialState: InitialState) {
  ...
  const participantsRequestS = fetchMoreParticipantsS
    .flatMapLatest(fetchParticipants)

  return Bacon.update(initialState.competitionState.adminStats, 
      [participantsRequestS], (iv, nv) => nv || {}
    )
}
...
function fetchParticipants() {
  const promise = Axios.get(config.apiEntrypoint + '/api/participants?d=' + Date.now())
    ...
    .then(nv => ({participants: toNickNameArray(nv), ...userDrinksObjectToDrinkCount(nv)}))
    .catch(console.error)
...
export function userDrinksObjectToDrinkCount(data: UserDrinksObject[]): {drinks: number, nonHolic: number, holic: number} {
