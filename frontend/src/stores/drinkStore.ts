...
export function drinkStore(initialState: InitialState) {
  ...
  return Bacon.update(initialState.competitionState.userDrinks,
      [drinkRequestS], (iv, nv) => [...iv, nv]
    )
}

function addDrink(drinkType: DrinkType) {
  const requestPromise = axios.post(config.apiEntrypoint + '/api/drinks', {drinkType}, {headers: {authorization: document.cookie.split('token=')[1].split(';')[0]}})
